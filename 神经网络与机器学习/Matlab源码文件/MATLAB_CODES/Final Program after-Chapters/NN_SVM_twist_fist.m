function err = NN_SVM_twist_fist(C)%[nsv, alpha, b0, data] = NN_SVM_twist_fist %(X,Y,C)
...
%%============== Step 0A: Generate twist_fist data ==========================
d1 = 0.8;
d2 = 0.5;
d3 = 0.2;
...
num_tr   = 200;  % # of training datasets
num_te   = 200; % # of testing datasets
num_samp = num_tr + num_te;% number of samples
fprintf('Support Vector Machine for Classification\n');
fprintf('_________________________________________\n');
fprintf('Generating twist_fist data ...\n');
fprintf('  ------------------------------------\n');
fprintf('  Points generated: %d\n',num_samp);
fprintf('  Twist fist data parameters: d1 = %2.1f,d2 = %2.1f,d3 = %2.1f\n',d1,d2,d3);
fprintf('  ------------------------------------\n');
seed=2e5;
rand('seed',seed);
data_shuffled = twist_fist(d1,d2,d3,num_samp);
%load fist_data;
warning off;


%%================== Step 0B: Initialization of SVM =======================
%C      = 2500;  % upper bound (non-separable case)
p1     = 0.3;    % sigma for RBF, width
...
mean0 = [mean(data_shuffled(1:2,:)')';0];         % mean of the original data
for i = 1:num_samp,
...
subplot(121);
box on;
title(['Training: C= ',num2str(C), ' \sigma = ', num2str(p1)]);
...
subplot(122);
box on;
title(['Testing: n_{test}= ',num2str(num_te), ' error rate = ', num2str((err/num_te)*100), '%']);
svcplot_test(X_tr,Y_tr,X_te,Y_pred,alpha,b0,epsilon,p1,mean0,max0);
xlabel('x');ylabel('y');
ylim([-d1,d1]);
%xlim([-d1,d1]);
% title(['Classification using SVM with dist = ',num2str(dist), ', radius = ',...
%        num2str(rad), ' and width = ',num2str(width), '(Testing)']);
